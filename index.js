...
const ACTIVE_GUILD_OPERATIONS = new Map();
const PROCESSED_COMMAND_MESSAGES = new Set();
const PROCESS_ID = `${process.pid}-${Date.now()}`;
const STARTUP_GRACE_PERIOD_MS = 20000;
const CROSS_PROCESS_LOCK_TTL_MS = 10 * 60 * 1000;
const COMMAND_GUARD_WINDOW_MS = 30000;
let clientReadyAt = 0;
...
const CONFIG_DISPLAY_LABELS = {
    logs: 'канал за логове',
    roles: 'канал за избор на роли',
...
function normalizeGuildState(state = {}) {
    return {
        ...
        locks: state.locks && typeof state.locks === 'object' && !Array.isArray(state.locks) ? state.locks : {},
        commandGuards: state.commandGuards && typeof state.commandGuards === 'object' && !Array.isArray(state.commandGuards) ? state.commandGuards : {}
    };
}
...
function getOperationKey(guildId, operation) {
    return `${guildId}:${operation}`;
}

function isOperationActive(guildId, operation) {
    return ACTIVE_GUILD_OPERATIONS.has(getOperationKey(guildId, operation));
}

function startOperation(guildId, operation) {
    ACTIVE_GUILD_OPERATIONS.set(getOperationKey(guildId, operation), Date.now());
}

function finishOperation(guildId, operation) {
    ACTIVE_GUILD_OPERATIONS.delete(getOperationKey(guildId, operation));
}

function markCommandAsProcessed(messageId) {
    PROCESSED_COMMAND_MESSAGES.add(messageId);
    setTimeout(() => {
        PROCESSED_COMMAND_MESSAGES.delete(messageId);
    }, 60000);
}

function isCommandAlreadyProcessed(messageId) {
    return PROCESSED_COMMAND_MESSAGES.has(messageId);
}

function isInStartupGracePeriod() {
    return clientReadyAt > 0 && Date.now() - clientReadyAt < STARTUP_GRACE_PERIOD_MS;
}

function cleanupExpiredGuildState(state) {
    const now = Date.now();

    if (state.locks) {
        Object.keys(state.locks).forEach(key => {
            const lock = state.locks[key];
            if (!lock || !lock.timestamp || now - lock.timestamp > CROSS_PROCESS_LOCK_TTL_MS) {
                delete state.locks[key];
            }
        });
    }

    if (state.commandGuards) {
        Object.keys(state.commandGuards).forEach(key => {
            const guard = state.commandGuards[key];
            if (!guard || !guard.timestamp || now - guard.timestamp > COMMAND_GUARD_WINDOW_MS) {
                delete state.commandGuards[key];
            }
        });
    }
}

function getCrossProcessLock(guildId, operation) {
    const state = getGuildState(guildId);
    cleanupExpiredGuildState(state);
    if (state.locks && state.locks[operation]) {
        return state.locks[operation];
    }
    return null;
}

function tryAcquireCrossProcessLock(guildId, operation) {
    const now = Date.now();
    let acquired = false;

    updateGuildState(guildId, state => {
        cleanupExpiredGuildState(state);
        const existingLock = state.locks?.[operation];
        if (!existingLock) {
            state.locks[operation] = {
                owner: PROCESS_ID,
                timestamp: now
            };
            acquired = true;
...
function refreshCrossProcessLock(guildId, operation) {
    updateGuildState(guildId, state => {
        cleanupExpiredGuildState(state);
        const existingLock = state.locks?.[operation];
        if (existingLock && existingLock.owner === PROCESS_ID) {
            state.locks[operation].timestamp = Date.now();
        }
    });
}

function releaseCrossProcessLock(guildId, operation) {
    updateGuildState(guildId, state => {
        cleanupExpiredGuildState(state);
        const existingLock = state.locks?.[operation];
        if (existingLock && existingLock.owner === PROCESS_ID) {
            delete state.locks[operation];
        }
    });
}

function hasRecentCommandGuard(guildId, commandName) {
    const state = getGuildState(guildId);
    cleanupExpiredGuildState(state);
    return Boolean(state.commandGuards?.[commandName]);
}

function setCommandGuard(guildId, commandName, messageId) {
    updateGuildState(guildId, state => {
        cleanupExpiredGuildState(state);
        state.commandGuards[commandName] = {
            owner: PROCESS_ID,
            messageId,
            timestamp: Date.now()
        };
    ...
    return new EmbedBuilder()
        ...
        .setDescription(
            [
                ...
                'Целта на сървъра е приятна атмосфера за игра, разговори, отборна координация и общностни активности за CS2, Call of Duty, Battlefield 6, GTA V и други гейминг вечери.',
                '',
                ...
                'Пишете по темата в съответните секции и не пренасяйте излишни драми или спорни теми в гейминг каналите.',
                '',
                ...
                'Публикувайте ясни покани за игра: игра, режим, брой места и дали търсите състезателна игра, нормална игра или гласова комуникация.',
                '',
                ...
                'Записвания, резултати и участие се правят честно. Фалшиви записвания, неверни резултати и умишлено саботиране не се приемат.',
                '',
                '**13. Наказанията се налагат по преценка на екипа.**',
                'При нарушения могат да се прилагат предупреждение, ограничен достъп, timeout, mute, изгонване или ban според тежестта и повторяемостта.',
                '',
                '**14. Ескалацията е стандартна, но не е задължително линейна.**',
                'По-леките нарушения обикновено минават през предупреждение, а по-сериозните могат директно да доведат до ограничаване на достъп или ban.',
                '',
...
async function handleResetServer(message) {
    if (isOperationActive(message.guild.id, 'setup') || isOperationActive(message.guild.id, 'reset')) {
        await message.reply('⏳ В момента вече се изпълнява настройка или нулиране на този сървър.');
        return;
    }

    if (getCrossProcessLock(message.guild.id, 'setup') || getCrossProcessLock(message.guild.id, 'reset')) {
        await message.reply('⏳ В момента вече се изпълнява настройка или нулиране на този сървър.');
        return;
    }
    ...
    collector.on('collect', async () => {
        ...
        if (!tryAcquireCrossProcessLock(message.guild.id, 'reset')) {
            await message.channel.send('⏳ Нулирането вече се изпълнява от друг активен bot process.');
            return;
        }

        startOperation(message.guild.id, 'reset');
        const statusMessage = await message.channel.send('⏳ Започвам изчистването за нова структура...');
        try {
            refreshCrossProcessLock(message.guild.id, 'reset');
            ...
            finishOperation(message.guild.id, 'reset');
            releaseCrossProcessLock(message.guild.id, 'reset');
        }
    });
...
async function handleSetupServer(message) {
    if (isOperationActive(message.guild.id, 'setup')) {
        await message.reply('⏳ Setup вече се изпълнява.');
        return;
    }

    if (isOperationActive(message.guild.id, 'reset')) {
        await message.reply('⏳ В момента тече нулиране на сървъра. Изчакай да приключи и опитай отново.');
        return;
    }

    if (getCrossProcessLock(message.guild.id, 'setup')) {
        await message.reply('⏳ Setup вече се изпълнява.');
        return;
    }

    if (getCrossProcessLock(message.guild.id, 'reset')) {
        await message.reply('⏳ В момента тече нулиране на сървъра. Изчакай да приключи и опитай отново.');
        return;
    }

    if (!tryAcquireCrossProcessLock(message.guild.id, 'setup')) {
        await message.reply('⏳ Setup вече се изпълнява от друг активен bot process.');
        return;
    }

    startOperation(message.guild.id, 'setup');
    const statusMessage = await message.reply('⏳ Изграждам новата гейминг структура на сървъра...');

    try {
        refreshCrossProcessLock(message.guild.id, 'setup');
        ...
        finishOperation(message.guild.id, 'setup');
        releaseCrossProcessLock(message.guild.id, 'setup');
    }
}
...
async function handleConfigCommand(message, args) {
    ...
    if (subcommand === 'show') {
        const lines = CONFIG_CHANNEL_KEYS.map(key => {
            ...
            return `• ${CONFIG_DISPLAY_LABELS[key] || key}: ${channel ? `<#${channel.id}>` : 'няма зададен канал'}`;
        });
    ...
    if (subcommand === 'auto') {
        ...
        await message.reply(`✅ Автоматичното разпознаване завърши. Запазени са ${updatedCount} връзки към канали.`);
        return;
    }
    ...
    if (subcommand === 'set') {
        ...
        await message.reply(`✅ Запазих ${CONFIG_DISPLAY_LABELS[channelKey] || channelKey} -> ${channel}.`);
        return;
    }
...
client.on('messageCreate', async message => {
    ...
    if (isCommandAlreadyProcessed(message.id)) return;

    markCommandAsProcessed(message.id);

    const args = message.content.slice(PREFIX.length).trim().split(/\s+/);
    const command = args.shift()?.toLowerCase();

    if (isInStartupGracePeriod() && (command === 'setup-server' || command === 'reset-server')) {
        await message.reply('⏳ Ботът току-що стартира. Изчакай няколко секунди и опитай отново.');
        return;
    }

    if ((command === 'setup-server' || command === 'reset-server') && hasRecentCommandGuard(message.guild.id, command)) {
        await message.reply('⏳ Командата вече беше приета преди малко. Изчакай текущото изпълнение да приключи.');
        return;
    }

    if (command === 'setup-server' || command === 'reset-server') {
        setCommandGuard(message.guild.id, command, message.id);
    }
